...
pub async fn execute(node: &Node, sender: &UnboundedSender<Result<Event, Infallible>>) -> anyhow::Result<Vec<Log>> {
    let mut logs = vec![];
    let config = OpenAIConfig::new().with_api_key("None").with_api_base("http://222.190.139.186:11436/v1");
    let client = Client::with_config(config);

    let request = CreateCompletionRequestArgs::default().model("qwen3:14b").n(1).prompt("你好呀").stream(true).max_tokens(1024_u32).build()?;
