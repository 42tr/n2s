  ...
  const updateNodesWithExecutionData = (execution: Execution) => {
    ...
    execution.logs.forEach((log: any) => {
      if (log.data) {
        ...
        if (nodeId) {
          let currentStatus = nodeStatusMap.get(nodeId) || { status: 'idle', output: '', error: '' };
          ...
          switch (type) {
            ...
            case 'output':
              if (logData) {
                currentStatus.output = typeof logData === 'string' ? logData : JSON.stringify(logData, null, 2);
              }
              break;
          }
      ...
      <div className="content">
        <div className="executions-panel">
          <div className="panel-header">
            <h2>执行记录</h2>
            <button onClick={loadExecutions} disabled={loading} className="refresh-btn">
              {loading ? '加载中...' : '刷新'}
            </button>
          </div>
